...
//        Mockito.when(depositRatesRepository.findAll(Mockito.any(dr.getClass()))).thenReturn(List.of(depRateC1001_25_36M));
    ...
    public void test1_FirstPeriodWithOutTransaction() {
        //расчета не происходит
        assertNull(lec.getFirstPeriodWithoutTransactionUTC());
    }

    @Test
    public void test2_NumberOfDeletedTransactions() {
        assertEquals(32, lec.getCalculatedStornoDeletedEntries().stream()
                .filter(entry -> entry.getStatus() == EntryStatus.DELETED)
                .count());
    }

    @Test
    public void test3_NumberOfAllTransactionsDeletedAndCalculated() {
    ...
    public void InitializeGeneraldata() {
        ...
        fact = getSC("FACT", ScenarioStornoStatus.ADDITION, user);
        usd = getCUR("USD");
    ...
    public void create_LD_1_NormalTestLD() {
        ...
        LocalDate.of(2017, 3, 31).datesUntil(LocalDate.of(2019, 11, 30), java.time.Period.ofMonths(1)).forEach(date ->
        {
            EntryID entryID = EntryID.builder()
                    .leasingDeposit_id(leasingDeposit1.getId())
                    .CALCULATION_TIME(ZonedDateTime.now())
                    .scenario(fact)
                    .period(periods.get(getDate(date.getDayOfMonth(), date.getMonthValue(), date.getYear())))
                    .build();
